...
export class OnscreenComponent implements OnDestroy {
    ...
    async createMeeting() {
        ...
        this.joinMeeting(name).then(() => {
            this.jitsiAPI.on('videoConferenceJoined', (ev: any) => {
                //console.log("ONSCREEN conference joined event:", ev);
                let us = ev.id;
                this.jitsiAPI.on('participantRoleChanged', (ev: any) => {
                    if (ev.id === us && ev.role === "moderator") {
    ...
    async joinMeeting(code: string) {
        ...
        this.jitsiAPI.on('participantJoined', (ev: any) => {
            //console.log("ONSCREEN participant joined:", ev);
            let participant = {
                id:          ev.id,
